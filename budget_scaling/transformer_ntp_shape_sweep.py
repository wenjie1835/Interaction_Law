...
class TrainCfg:
    ...
    # 0 = disabled (use full agop_batch for each JVP). >0 caps micro-batch for JVP VRAM.
...
def train_one_model(
    ...
    # Full-dataset CE (no per-batch print in-loop except log_every below; can be tens
    # of thousands of batches for Chinchilla-scaled D — hours of wall time is normal).
    # CUDA may delay SIGINT until sync — do not Ctrl+C twice.
    print(
        "    [train] final CE on train/val/test (full loaders; may take minutes) …",
        flush=True,
    )
    tr_ce  = evaluate_ntp(
        model, train_loader, device, log_every=500, log_label="final CE train",
    )
    val_ce = evaluate_ntp(
        model, val_loader, device, log_every=500, log_label="final CE val",
    )
    te_ce = evaluate_ntp(
        model, test_loader, device, log_every=500, log_label="final CE test",
    )
...
def run_shape_sweep_for_n(
    *,
    ...
    for depth in depths:
        ...
        print("    [AGOP] estimating projected AGOP (GPU-heavy; progress not logged per probe) …", flush=True)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Cap JVP batch slice size for AGOP (0 = use full --agop_batch). Low values reduce peak VRAM.",
    )
    parser.add_argument(
        "--agop_low_vram",
        action="store_true",
        help="Constrain AGOP settings for large models: cap batch/proj samples, enable micro-batching (override with explicit flags as needed).",
    )
    parser.add_argument(
        "--only_depth",
        type=int,
    ...
    print(
        f"AGOP: batch={cfg.agop_batch}  proj_samples={cfg.agop_proj_samples}  "
