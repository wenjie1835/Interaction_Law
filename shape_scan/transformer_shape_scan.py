...
AOFE-ratio. Training uses **validation early stopping** after the token budget
is met: once at least the Chinchilla ``base_steps`` have run, stop if validation
loss fails to improve for ``--fit-patience`` consecutive evals (see
``--max-train-factor`` for the hard step cap). The checkpoint with best
validation loss is restored before final metrics and AOFE.
Finally, compute correlations between loss and the two AOFE metrics.
...
Offline / mirror notes (WikiText)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
HuggingFace can fail with ``Network is unreachable``; the legacy Metamind S3 URL often
redirects to a bucket hostname whose TLS certificate no longer matches, producing
``SSLCertVerificationError``. Mitigations:

- **WikiText-103:** point ``--data-dir`` at the repo's ``Interaction_Law/data`` (or any
  folder that already contains ``wikitext103_train.bin`` and
  ``wikitext103_validation.bin`` from ``transformer_ntp_shape_sweep``) and pass
  ``--dataset wikitext-103``; no zip download is attempted.
- **WikiText-2:** set ``HF_ENDPOINT=https://hf-mirror.com`` before running (if your
  HuggingFace client supports it), or pass ``--wikitext-zip <url>``, or set env
  ``WIKITEXT_ZIP`` to a direct ``wikitext-2-v1.zip`` URL.
...
Additionally, each row records ``agop_mean_cos2`` computed directly from the same
embedding-channel AGOP-like matrix used for AOFE/AOFE-ratio.
...
# Extra HTTPS URLs tried after Metamind S3 (that URL often redirects to a host whose TLS name no longer matches).
WIKITEXT_ZIP_EXTRA: Dict[str, List[str]] = {
    "wikitext-2": [
        "https://huggingface.co/datasets/Salesforce/wikitext/resolve/main/wikitext-2-v1.zip",
        "https://hf-mirror.com/datasets/Salesforce/wikitext/resolve/main/wikitext-2-v1.zip",
    ],
    "wikitext-103": [
        "https://huggingface.co/datasets/Salesforce/wikitext/resolve/main/wikitext-103-v1.zip",
        "https://hf-mirror.com/datasets/Salesforce/wikitext/resolve/main/wikitext-103-v1.zip",
    ],
}
...
@torch.no_grad()
def summarize_agop_metrics(agop: torch.Tensor) -> Dict[str, float]:
    """
    Metrics computed on the same AGOP-like matrix over embedding channels.

    Returns:
        aofe            = off-diagonal Frobenius energy
        aofe_ratio      = off-diagonal energy / total Frobenius energy
        agop_mean_cos2  = weighted mean cos^2 from the AGOP Gram-like matrix
    """
    return {
        "aofe": offdiag_energy_from_matrix(agop),
        "aofe_ratio": offdiag_energy_ratio_from_matrix(agop),
...
def _wikitext_zip_url_list(dataset: str, override: Optional[str]) -> List[str]:
    """Ordered list of zip URLs: CLI/env override, legacy S3, then HuggingFace / mirror fallbacks."""
    out: List[str] = []
    ...
    push(override)
    push(os.environ.get("WIKITEXT_ZIP"))
    push(WIKITEXT_URLS.get(dataset))
    for u in WIKITEXT_ZIP_EXTRA.get(dataset, []):
        push(u)
    return out


WIKITEXT_OFFLINE_DOWNLOAD_HINT = """

Offline fixes (WikiText downloads often fail behind firewalls / broken Metamind TLS):
  • WikiText-2: copy ``wikitext-2-v1.zip`` onto this machine and place at
        <data-dir>/wikitext-2-v1.zip
    Or set ``WIKITEXT_ZIP`` / ``python ... --wikitext-zip PATH`` where PATH is an **absolute or relative
    path on disk** (not only a URL)—the loader will ``copy`` that file into the cache.
  • WikiText-103 (no WikiText-2 zip needed): point ``--data-dir`` at repo
        Interaction_Law/data
    where ``wikitext103_train.bin`` and ``wikitext103_validation.bin`` (or caches under ``wikitext-103/``) exist,
    and run ``--dataset wikitext-103``.
"""


def download_wikitext_zip(urls: List[str], dest: Path) -> None:
    """Try each URL until one succeeds; removes a partial ``dest`` on failure."""
    errors: List[str] = []
    for url in urls:
        try:
            ...
            if src.is_file():
                ...
                print(f"Using local WikiText zip (copied): {src} → {dest}")
                return
            raise FileNotFoundError(f"not a reachable URL path and no file: {url!r}")
            ...
            if dest.exists():
                try:
                    ...
                    pass
    msg = "Could not download WikiText zip. Tried:\n" + "\n".join(errors) + WIKITEXT_OFFLINE_DOWNLOAD_HINT
    raise RuntimeError(msg)


def _try_materialize_wikitext103_from_flat_bins(
    data_dir: Path,
    dataset: str,
    train_txt: Path,
    valid_txt: Path,
    ...
    """
    Reuse plain UTF-8 byte dumps ``wikitext103_train.bin`` / ``wikitext103_validation.bin``
    (same layout as ``experiments/transformer_ntp_shape_sweep.py``) under ``data_dir``.
    Avoids HuggingFace and broken Metamind S3 when you already have those files.
    """
    if dataset != "wikitext-103":
        return
    if train_txt.exists() and valid_txt.exists():
        return
    train_src = data_dir / "wikitext103_train.bin"
    valid_src = data_dir / "wikitext103_validation.bin"
    if not train_src.is_file() or not valid_src.is_file():
        return
    train_txt.parent.mkdir(parents=True, exist_ok=True)
    print(
        f"Using local corpora {train_src.name} / {valid_src.name} → "
        f"{train_txt.relative_to(data_dir)} & {valid_txt.relative_to(data_dir)} (no download)."
    )
    train_txt.write_bytes(train_src.read_bytes())
    valid_txt.write_bytes(valid_src.read_bytes())
...
def prepare_wikitext_bytes(
    data_dir: Path, dataset: str, *, wikitext_zip_override: Optional[str] = None
) -> Tuple[Path, Path]:
    ...
    _try_materialize_wikitext103_from_flat_bins(data_dir, dataset, train_txt, valid_txt)
    ...
    if not train_txt.exists() or not valid_txt.exists():
        ...
        if not hf_ok:
            if not zip_path.exists():
                urls = _wikitext_zip_url_list(dataset, wikitext_zip_override)
                print(f"Downloading {dataset} (trying {len(urls)} zip URL(s)) …")
                download_wikitext_zip(urls, zip_path)
...
def prepare_dataset_bytes(
    ...
    *,
    ...
    wikitext_zip_override: Optional[str] = None,
) -> Tuple[Path, Path]:
    if dataset in WIKITEXT_URLS:
        return prepare_wikitext_bytes(
            data_dir, dataset, wikitext_zip_override=wikitext_zip_override
        )
...
def train_one_model(
    ...
    weight_decay: float,
    warmup_steps: int,
    max_steps_debug: Optional[int],
    max_train_factor: float,
    fit_patience: int,
    device: torch.device,
    compile_model: bool,
) -> Dict[str, float]:
    ...
    base_steps = min(planned_steps, max_steps_debug) if max_steps_debug is not None else planned_steps
    if max_train_factor < 1.0:
        raise ValueError("max_train_factor must be >= 1.0")

    lr_horizon_steps = max(
        base_steps,
        int(math.ceil(max_train_factor * base_steps)),
    )

    if lr_horizon_steps <= 0:
        raise ValueError("lr_horizon_steps must be positive (check train_tokens and batch settings).")
    ...
    print(
        f"  base_steps={base_steps} planned_steps(planned_tokens)={planned_steps} "
        f"lr_horizon_steps={lr_horizon_steps} tokens_per_step={tokens_per_step} "
        f"fit_patience={fit_patience} max_train_factor={max_train_factor}"
    )

    for step in range(lr_horizon_steps):
        lr_now = cosine_lr(step, lr_horizon_steps, lr, min_lr, warmup_steps)
        ...
        is_eval_step = (
            step == 0 or (step + 1) % eval_interval == 0 or (step + 1) >= lr_horizon_steps
        )
        if is_eval_step:
            ...
            print(
                f"    step={step + 1:5d}/{lr_horizon_steps} "
                f"train_loss={last_train_loss:.4f} val_loss={val_loss:.4f} "
                f"best_val={best_val_loss:.4f} stale={stale} lr={lr_now:.2e} elapsed={elapsed / 60:.1f}m"
            )

            should_stop = (
                fit_patience > 0 and (step + 1) >= base_steps and stale >= fit_patience
            )
            if should_stop:
                print(f"    [early-stop] fit_patience={fit_patience} at step {step + 1} (fitted / plateau)")
                done_fit_stop = True
                break
    ...
    return {
        ...
        "lr_horizon_steps": float(lr_horizon_steps),
        "tokens_per_step": float(tokens_per_step),
        "effective_train_tokens": float(executed_steps * tokens_per_step),
        "final_train_loss": float(last_train_loss),
        "final_val_loss": float(final_val_loss),
        "best_val_loss": float(best_val_loss),
        "stopped_fit_patience": float(1.0 if done_fit_stop else 0.0),
    }


def compute_embedding_channel_agop_metrics(
    model: TinyGPT,
    valid_data: np.ndarray,
    batch_size: int,
    block_size: int,
    ...
    return summarize_agop_metrics(agop)
...
def compute_correlations(rows: List[Dict[str, object]]) -> Dict[str, Dict[str, float]]:
    ...
    agop_c2 = np.asarray([float(r["agop_mean_cos2"]) for r in rows], dtype=np.float64)
    out: Dict[str, Dict[str, float]] = {
        ...
        "final_val_loss_vs_agop_mean_cos2": {
            "pearson": pearsonr(loss, agop_c2),
            "spearman": spearmanr(loss, agop_c2),
        },
        "aofe_vs_agop_mean_cos2": {
            "pearson": pearsonr(aofe, agop_c2),
            "spearman": spearmanr(aofe, agop_c2),
        },
        "aofe_ratio_vs_agop_mean_cos2": {
            "pearson": pearsonr(ratio, agop_c2),
            "spearman": spearmanr(ratio, agop_c2),
        },
    }
    return out
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path("./data"),
        help="WikiText: directory for caches / zips. Use repo Interaction_Law/data for pre-made wikitext103_*.bin.",
    )
    parser.add_argument(
        "--wikitext-zip",
        type=str,
        default=None,
        help="WikiText only: explicit URL for the -v1.zip (overrides env WIKITEXT_ZIP). Use when HF/S3 fail.",
    )
    parser.add_argument(
    ...
    parser.add_argument("--max-steps", type=int, default=None, help="Cap Chinchilla step count (debug); full token budget is base_steps = min(planned, this).")
    parser.add_argument(
        "--max-train-factor",
        type=float,
        default=1.5,
        help="Upper step cap = ceil(max_train_factor × base_steps); training may stop earlier via --fit-patience.",
    )
    parser.add_argument(
        "--fit-patience",
        type=int,
        default=10,
        help=(
            "After base_steps, stop if validation loss does not improve for this many consecutive evals "
            "(eval every --eval-interval optimizer steps). "
            "0 disables early stopping (always run max_train_factor × base_steps)."
        ),
    )
    ...
    train_bin, valid_bin = prepare_dataset_bytes(
        ...
        wikitext_zip_override=args.wikitext_zip,
    )
    ...
    for cfg, params in configs:
        ...
        stats = train_one_model(
            ...
            max_steps_debug=args.max_steps,
            max_train_factor=args.max_train_factor,
            fit_patience=args.fit_patience,
            device=device,
            compile_model=args.compile,
        )
        agop_metrics = compute_embedding_channel_agop_metrics(
            model=model,
            valid_data=valid_data,
            batch_size=args.batch_size,
            block_size=args.block_size,
        ...
        print(
            f"  AOFE={agop_metrics['aofe']:.6e} AOFE-ratio={agop_metrics['aofe_ratio']:.6f} "
            f"AGOP_mean_cos2={agop_metrics['agop_mean_cos2']:.4f} "
